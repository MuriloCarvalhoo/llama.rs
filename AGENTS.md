...
When the repository has a `.codegraph/` directory, use CodeGraph before grep, find, or reading files to locate or understand code:

- **MCP:** use `codegraph_explore` for relevant source, call paths, and impact.
- **Shell fallback:** `codegraph explore "<symbol names or question>"` provides the same context.
